...
- **ViewmodelComponent** (`src/components/viewmodel_component.gd`) -- First-person weapon display. Parented to Camera3D, manages sword and shield meshes (extracted from Knight.glb at runtime). Procedural Tween-based animations: `play_attack()`, `play_attack_visual()` (no hitbox signal, used by debug loop), `raise_shield()`/`lower_shield()`, `play_death()`, `set_bobbing()`. Signals: `attack_hit_point`, `attack_finished`, `death_finished`. All durations, positions, and attack direction offsets are `@export`.
