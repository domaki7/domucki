...
- **HitboxComponent** (`src/components/hitbox_component.gd`) -- Area3D that deals damage on overlap. `@export damage`. Starts deactivated. Call `activate()` / `deactivate()` to toggle CollisionShape3D. Set `monitoring = false`, `monitorable = true` so hurtboxes detect it. Emits `hit(hurtbox)`. Call `get_collision_shape()` to access the CollisionShape3D child. Has a swing arc tween system: call `play_swing_tween()` to move the CollisionShape3D through a 3-phase arc (windup → hit → recovery) matching attack animations. `@export` offsets (`swing_windup_offset`, `swing_hit_offset`) and durations (`swing_windup_duration`, `swing_hit_duration`, `swing_recovery_duration`) allow per-instance tuning. Call `reset_position()` on state exit to snap back to idle if interrupted. Call `update_idle_position(pos)` to change the base position (used by debug GUI).
- **HurtboxComponent** (`src/components/hurtbox_component.gd`) -- Area3D that receives damage. `@export health_component` (auto-discovers from siblings if unset). Auto-discovers StaminaComponent from siblings. Set `monitoring = true`, `monitorable = false`. On overlap with HitboxComponent: if `is_blocking` and stamina available, spends stamina and emits `damage_blocked(hitbox)` instead of dealing damage. Otherwise emits `hurt(hitbox)` and calls `health_component.take_damage()`. Call `get_collision_shape()` to access the CollisionShape3D child.
- **StaminaComponent** (`src/components/stamina_component.gd`) -- Stamina resource tracking. `@export max_stamina`, `sprint_drain_rate`, `regen_rate`, `regen_delay`, `jump_cost`, `attack_cost`, `block_cost`. Set `is_sprinting = true` to drain continuously; otherwise regenerates after `regen_delay`. Call `spend(amount)` for flat costs, `can_spend(amount)` to query. Signals: `stamina_changed(old_value, new_value)`, `stamina_depleted`.
- **ViewmodelComponent** (`src/components/viewmodel_component.gd`) -- First-person weapon display. Parented to Camera3D, manages sword and shield meshes (extracted from Knight.glb at runtime). Procedural Tween-based animations: `play_attack()`, `play_attack_visual()` (no hitbox signal, used by debug loop), `raise_shield()`/`lower_shield()`, `play_death()`, `set_bobbing()`. Signals: `attack_hit_point`, `attack_finished`, `death_finished`. All durations, positions, scales, and attack direction offsets are `@export`. Sword and shield each have two uniform float scale exports: `*_mesh_scale` (mesh only) and `*_pivot_scale` (pivot + animation reach).
