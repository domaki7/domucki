...
- **HitboxComponent** (`src/components/hitbox_component.gd`) -- Area3D that deals damage on overlap. `@export damage`. Starts deactivated. Call `activate()` / `deactivate()` to toggle CollisionShape3D. Set `monitoring = false`, `monitorable = true` so hurtboxes detect it. Emits `hit(hurtbox)`. Call `get_collision_shape()` to access the CollisionShape3D child.
